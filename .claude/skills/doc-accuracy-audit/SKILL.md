...
## Attesto's own doc set, as of 2026-09-18

The files this actually applies to: `README.md`, `DECISIONS.md`,
`app/legal/page.tsx`, `docs/try-it-guide.md`, `docs/video-script.md`,
and the "How it works"/"API reference" copy in
`app/components/attesto/`. A real Step 3-shaped check was already run
this project's own session on 2026-09-17 (curling `/v1/skill-check`,
`/v1/metrics` against both `attesto-api.fly.dev` and `attesto.xyz`,
confirming the documented x402 flow actually works end-to-end with a
real funded wallet) — that's the standard to repeat before any future
doc claim about the payment flow is trusted again, not a one-time
