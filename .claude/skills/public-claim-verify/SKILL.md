...
## Attesto-specific examples already worth re-checking with this discipline

Two concrete claims already live in this project's public copy that a
future publish pass should re-verify the same way, not assume settled
forever: the Vouch402 prior-art disclosure (README, DECISIONS.md, the
legal page all state it — each is its own claim, verify each file
separately, not just once); and any specific transaction/receipt cited
in the pitch video or a social post once one exists — cite the real
on-chain signature and explorer link (`[VERIFIED: <tx signature>,
<date>]`), never a design-target number presented as an achieved result.
